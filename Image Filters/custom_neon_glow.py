...
# Function to apply a neon glow filter
def apply_neon_glow_filter(image):
    # Enhance the colors with a neon effect
    neon_filter = np.array([[1.5, 0.2, 0.2],
                            [0.2, 1.5, 0.2],
                            [0.2, 0.2, 1.5]])
    neon_image = cv2.transform(image, neon_filter)
    neon_image = np.clip(neon_image, 0, 255)  # Ensure pixel values are valid

    # Increase contrast for a more striking effect
    alpha = 1.4  # Contrast control
    beta = 20    # Brightness control
    neon_image = cv2.convertScaleAbs(neon_image, alpha=alpha, beta=beta)
...
neon_glow_image = apply_neon_glow_filter(image)

# Save and display the neon glow image
neon_glow_image_path = 'neon_glow_image.jpg'
cv2.imwrite(neon_glow_image_path, neon_glow_image)

plt.imshow(cv2.cvtColor(neon_glow_image, cv2.COLOR_BGR2RGB))
plt.axis('off')
plt.title('Neon Glow Image')
plt.show()

print(f'Neon glow image created and saved as "{neon_glow_image_path}".')
